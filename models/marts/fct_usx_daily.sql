...
final as (
    select
        ...
        sum(daily_redeems) over (partition by symbol order by date ) as cumulative_redeems,
        sum(daily_netflow) over (partition by symbol order by date ) as tvl_collateral
    from daily
)
